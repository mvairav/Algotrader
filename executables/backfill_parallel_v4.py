...
async def process_task_list(ibkr, task_list:List[TaskInfo]):
    ...
    for task_info in task_list:
        ...
        task_info.data_logger = data_logger
        task_info.req_count_for_contract = task_info.req_count_for_contract + 1 if task_info.req_count_for_contract else 1
        task_info.return_bars = True

        task_info.task = asyncio.create_task(submit_tasks(task_info, ibkr), name=f"{task_info.contract.localSymbol}_{task_info.start.date()}_{task_info.bar_size}")
    ...
    task = progress.add_task("Received", total=len(task_list))
...
async def main():
    ...
    start_time = time.perf_counter()
    contracts = await ibkr.get_option_chains_new(symbol, exchange, currency,strike_price_range,expiry_months) 
    end_time = time.perf_counter()
